-- 레시피 자동 채움을 매일 1회 → 10분마다 확인으로 변경 (함수가 최근 10분 내 저장이면 건너뜀)
do $$
begin
    ...
    if exists (select 1 from cron.job where jobname = 'nmf-recipe-fill-10min') then
        perform cron.unschedule('nmf-recipe-fill-10min');
    end if;
    perform cron.schedule(
        'nmf-recipe-fill-10min',
        '*/10 * * * *',
        $cron$
