...
export type Missing = { menu: string; comp: string; used: string[]; similar: string[]; cells: { ym: string; key: string }[] };
...
export function missingList(S: State): Missing[] {
  ...
  for (const [ym, m] of Object.entries(S.menus || {}) as [string, Record<string, string>][]) {
    for (const [k, v] of Object.entries(m)) {
      ...
      const o = out[v] || (out[v] = { menu: v, comp: SLOT_COMP[+ci] || "주찬", used: [], similar: [], cells: [] });
      o.used.push(`${ym}-${String(d).padStart(2, "0")} ${meal}`); o.cells.push({ ym, key: k });
    }
  }
  for (const [menu, comp] of Object.entries(emptyRecipes(S))) { const o = out[menu] || (out[menu] = { menu, comp, used: [], similar: [], cells: [] }); o.used.push("레시피 탭(재료 없음)"); }
  const names = [...have];
...
export type Recipe = { menu: string; comp?: string; allergy?: string; source?: string; method?: string; ask?: { question?: string; rename?: string[] } | null; items: { item: string; qty: number; unit?: string; storage?: string; loss?: number; form?: string }[] };
/** 레시피를 상태에 병합. 반환: 추가된 음식 이름 */
export function mergeRecipes(S: State, recipes: Recipe[], today: string, refs: Record<string, Ref | null> = {}): { added: string[]; skipped: string[] } {
  const have = recipeNames(S); const added: string[] = [], skipped: string[] = [];
  S.methods = S.methods || {}; S.sources = S.sources || {}; S.recipeMeta = S.recipeMeta || {}; S.recipeAsk = S.recipeAsk || {};
  for (const R of recipes || []) {
    ...
    if (R.method) S.methods[R.menu] = normalizeMethod(R.method); if (R.source) S.sources[R.menu] = R.source;
    ...
    const renames = (R.ask?.rename || []).filter((n) => n && n !== R.menu).slice(0, 3);
    if (R.ask?.question || renames.length) S.recipeAsk[R.menu] = { ask: R.ask?.question || `'${R.menu}' 이름이 정확한지 확인해 주세요.`, date: today, options: renames.map((n) => ({ label: `'${n}'(으)로 이름 바꾸기`, kind: "rename", to: n })) };
    else if (R.menu in refs && !refs[R.menu]) S.recipeAsk[R.menu] = { ask: "만개의레시피에서 같은 이름을 찾지 못해 일반적인 급식 레시피로 작성했습니다. 재료·분량을 확인해 주세요.", date: today, options: [] };
  }
...
export function askSimilar(S: State, list: Missing[], today: string): string[] {
  S.recipeAsk = S.recipeAsk || {}; const out: string[] = [];
  for (const m of list) { if (!m.similar.length || S.recipeAsk[m.menu]) continue;
    S.recipeAsk[m.menu] = { ask: `'${m.menu}'은(는) 이미 있는 '${m.similar.join("', '")}'과 이름이 비슷합니다. 같은 음식이면 식단표 이름을 바꿔 주세요. 다른 음식이면 레시피를 직접 넣어 주세요.`, date: today,
      options: m.similar.map((to) => ({ label: `식단표를 '${to}'(으)로 바꾸기`, kind: "menu", to, cells: m.cells })) };
    if (!(S.recipes || []).some((r: { menu: string }) => r.menu === m.menu)) S.recipes.push({ comp: m.comp, menu: m.menu, item: "", qty: 100, unit: "g", storage: "냉장", loss: 0.03, form: "원물", method: "", allergy: "" });   // 앱에서 안내가 보이도록 빈 레시피 한 줄
    out.push(m.menu); }
  return out;
}
...
export type Ref = { url: string; title: string; ingredients: string; steps: string[] };
...
export async function fetchReference(menu: string): Promise<Ref | null> {
  try {
    ...
    const r = await fetch("https://www.10000recipe.com/recipe/list.html?q=" + encodeURIComponent(q), { headers: UA, signal: AbortSignal.timeout(8000) });
    if (!r.ok) return null;
    const id = (await r.text()).match(/\/recipe\/(\d{6,8})/)?.[1]; if (!id) return null;
    const url = "https://www.10000recipe.com/recipe/" + id;
    const p = await fetch(url, { headers: UA, signal: AbortSignal.timeout(8000) }); if (!p.ok) return { url, title: "", ingredients: "", steps: [] };
    const t = await p.text();
    const title = strip(t.match(/<title>([^<]*)/)?.[1] || "").replace(/-.*$/, "").trim();
    const ingredients = strip(t.match(/<div class="ready_ingre3"[\s\S]*?<div class="view_step/)?.[0] || "").replace(/구매/g, "").slice(0, 700);
    const steps = [...t.matchAll(/id="stepdescr\d+"[\s\S]*?<div class="media-body">([\s\S]*?)<\/div>/g)].map((m) => strip(m[1])).filter(Boolean).slice(0, 12);
    return { url, title, ingredients, steps };
  } catch { return null; }
}

const EXAMPLE = `{"recipes":[{"menu":"오징어무침","comp":"부찬","allergy":"오징어, 밀, 대두","source":"https://www.10000recipe.com/recipe/7021555",
...
export function buildPrompt(list: Missing[], refs: Record<string, Ref | null> = {}): string {
  const refText = (m: Missing) => { const r = refs[m.menu]; if (!r) return `- ${m.menu} | 구성: ${m.comp} | 참고자료 없음 → source 는 "일반 급식 레시피"`;
    return `- ${m.menu} | 구성: ${m.comp} | source: ${r.url}\n  참고 제목: ${r.title}\n  참고 재료: ${r.ingredients.slice(0, 500)}\n  참고 조리: ${r.steps.join(" / ").slice(0, 900)}`; };
  return [
    "당신은 한국 공장 구내식당(단체급식) 영양사입니다. 아래 음식들의 1인 분량 레시피를 JSON 으로만 출력하세요. 설명 문장·마크다운은 쓰지 마세요.",
    "출력 형식과 문체는 아래 예시와 똑같이 맞추세요:",
    EXAMPLE,
    "규칙:",
    "- menu 는 주어진 이름과 글자 그대로 동일. comp 는 주어진 구성.",
...
export function normalizeMethod(m: string): string { const t = String(m || "").trim(); if (!t || t.includes("\n")) return t; return t.replace(/\s+(?=\d{1,2}\.\s)/g, "\n"); }
