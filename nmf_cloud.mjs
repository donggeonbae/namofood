...
function getPw() {
  ...
  throw new Error(
    "비밀번호가 없습니다. NMF_PW 환경변수 또는 ~/.namofood_pw 파일을 만들어 주세요.",
  );
}
const H = {
  apikey: KEY,
...
async function fetchState(pw) {
  const r = await fetch(`${api}?id=eq.${ROOM}&select=data,updated_at`, {
    headers: H,
  });
  ...
  return {
    S: JSON.parse(await decryptText(pw, rows[0].data)),
    updated_at: rows[0].updated_at,
  };
}
function recipeNames(S) {
  const s = new Set();
...
function missingList(S) {
  ...
  const out = {};
  for (const [ym, m] of Object.entries(S.menus || {})) {
    for (const [k, v] of Object.entries(m)) {
      const [d, meal, ci] = k.split("|");
      if (ci === "n" || !v || have.has(v)) {
        continue;
      }
      const o = out[v] || (out[v] = { menu: v, comp: slotComp(ci), used: [] });
      o.used.push(`${ym}-${String(d).padStart(2, "0")} ${meal}`);
    }
  }
  for (const r of S.recipes || []) {
    if (!r.menu || have.has(r.menu)) continue;
    const o = out[r.menu] ||
      (out[r.menu] = { menu: r.menu, comp: r.comp || "주찬", used: [] });
    if (!o.used.includes("레시피 탭(재료 없음)")) {
...
async function main() {
  ...
  if (cmd === "missing") {
    ...
    console.log(
      JSON.stringify(
        { updated_at, count: recipeNames(S).size, missing: missingList(S) },
        null,
        1,
      ),
    );
  ...
  if (cmd === "backup") {
    const { S } = await fetchState(pw);
    fs.writeFileSync(arg || "namofood_backup.json", JSON.stringify(S), "utf8");
  ...
  if (cmd === "apply") {
    ...
    const inp = JSON.parse(fs.readFileSync(arg, "utf8"));
    const today = new Date().toISOString().slice(0, 10);
    const { S, updated_at } = await fetchState(pw);
    if (Date.now() - new Date(updated_at).getTime() < 90000) {
      throw new Error(
        "누군가 90초 안에 저장했습니다. 잠시 뒤 다시 시도하세요.",
      );
    }
    const have = recipeNames(S);
    const added = [], skipped = [];
    ...
    for (const R of inp.recipes || []) {
      if (!R.menu || !Array.isArray(R.items) || !R.items.length) {
        skipped.push(R.menu || "?");
        continue;
      }
      if (have.has(R.menu)) {
      ...
      if (R.method) S.methods[R.menu] = R.method;
    ...
    const blob = await encryptText(pw, JSON.stringify(S));
    const at = S.updatedAt;
    const r = await fetch(`${api}?on_conflict=id`, {
      method: "POST",
      headers: { ...H, Prefer: "resolution=merge-duplicates,return=minimal" },
      body: JSON.stringify([{ id: ROOM, data: blob, updated_at: at }, {
    ...
    if (!r.ok) {
      throw new Error(
        "저장 실패 " + r.status + " " + (await r.text()).slice(0, 200),
      );
    }
    console.log(JSON.stringify({ added, skipped, saved_at: at }));
    return;
  }
  console.log(
    "사용법: node nmf_cloud.mjs missing | apply <json> | backup <out.json>",
  );
}
main().catch((e) => {
